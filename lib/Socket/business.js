...
const makeMessagesRecvSocket = (config) => {
    ...
    const processNotification = async (node) => {
        ...
        switch (nodeType) {
            ...
            case 'link_code_companion_reg':
                // FIX: WhatsApp puede enviar el payload directamente en el notification node
                // o dentro de un child con el mismo nombre. Intentamos el child primero.
                logger.info({
                    contentTags: Array.isArray(node.content) ? node.content.map(c => c.tag) : 'empty'
                }, '[PAIRING] link_code_companion_reg received');

                let linkCodeCompanionReg = (0, WABinary_1.getBinaryNodeChild)(node, 'link_code_companion_reg');
                if (!linkCodeCompanionReg) {
                    logger.warn('[PAIRING] child not found, using node directly');
                    linkCodeCompanionReg = node;
                }
                ...
                try {
                    ...
                    logger.info('[PAIRING] sending companion_finish...');
                    ...
                    logger.info('[PAIRING] companion_finish sent OK');
                    authState.creds.registered = true;
                    ev.emit('creds.update', authState.creds);
                } catch (pairingError) {
                    // FIX: antes el error se tragaba silenciosamente dentro del mutex
                    logger.error({ err: pairingError }, '[PAIRING] error processing link_code_companion_reg');
                    throw pairingError;
                }
                break;
        }
    ...
    const handleNotification = async (node) => {
        ...
        // LOG para debug de pairing
        logger.info({
            type: node.attrs.type,
            from: remoteJid,
            childTags: Array.isArray(node.content) ? node.content.map(c => c.tag) : []
        }, '[PAIRING] notification received');
        ...
        try {
            ...
            // FIX: propagar el error en lugar de tragarlo silenciosamente
            logger.error({ err, nodeType: node.attrs.type }, '[PAIRING] error in handleNotification');
            onUnexpectedError(err, 'handleNotification');
        }
        finally {
            await sendMessageAck(node);
        }
