...
const generateWAMessageContent = async (message, options) => {
    ...
    if ('contextInfo' in message && !!message.contextInfo) {
        ...
        // Fix: si externalAdReply tiene thumbnailUrl pero no thumbnail, lo fetchea como Buffer
        // Esto soluciona el problema del mensaje angosto en versiones nuevas de Baileys
        if (message.contextInfo?.externalAdReply?.thumbnailUrl && !message.contextInfo?.externalAdReply?.thumbnail) {
            try {
                const res = await axios_1.default.get(message.contextInfo.externalAdReply.thumbnailUrl, { responseType: 'arraybuffer' });
                message.contextInfo.externalAdReply.thumbnail = Buffer.from(res.data);
            } catch {}
        }
